...
2) "abbb"
```
Note that in the range queries we prefixed the min and max elements identifying the range with the special characters [ and (. This prefixes are mandatory(强制性的), and they specify if the elements of the range are inclusive or exclusive. So the range [a (b means give me all the elements lexicographically between a inclusive and b exclusive, which are all the elements starting with a.
...
An interesting application of indexing is completion.(索引的一个有趣的应用是 补全) Completion is what happens when you start typing your query into a search engine: the user interface will anticipate（预测） what you are likely typing, providing common queries that start with the same characters.
...
#### Adding frequency into the mix （加入频率）
The above approach is a bit naive(有些幼稚), because all the user searches are the same in this way. In a real system we want to complete strings according to their frequency: very popular searches will be proposed with an higher probability compared to search strings typed very rarely.
...
A refinement to this algorithm is to pick entries in the list according to their weight: the higher the score, the less likely entries are picked in order to decrement its score, or evict them.（此算法的一种改进是根据列表的权重选择列表中的条目：分数越高，为了降低其分数或将其逐出，选择的可能性就越小。）
...
#### Adding auxiliary(辅助的) information in the index
...
1) "mykey:myvalue"
```
Then we extract the part after the colon to retrieve the value. However a problem to solve in this case is collisions.（碰撞） The colon character may be part of the key itself（冒号可以就是key的一部分）, so it must be chosen in order to never collide with the key we add.
...
#### Numerical padding
Lexicographical indexes may look like good only when the problem at hand is to index strings. Actually it is very simple to use this kind of index in order to perform indexing of arbitrary precision numbers.（任意精度数字）
...
Storing numbers in decimal may use too much memory（以十进制存储数字可能会占用过多内存）. An alternative approach is just to store numbers, for example 128 bit integers, directly in their binary form. However for this to work, you need to store the numbers in big endian format, so that the most significant bytes are stored before the least significant bytes. This way when Redis compares the strings with memcmp(), it will effectively sort the numbers by their value.

Keep in mind that data stored in binary format is less observable for debugging, harder to parse and export. So it is definitely a trade off.（请记住，以二进制格式存储的数据在调试时很难观察到，更难以解析和导出。因此，这绝对是一个权衡。）
### Composite indexes
...
### Representing and querying graphs using an hexastore (使用hexastore表示和查询图)
...
#### summary
使用 ZRANGEBYSCORE，ZRANGE，ZRANGEBYLEX
