...
与key的相关命令，对所有类型都有效
exists del type expire ttl pexpire pttl
...
Redis lists are implemented via Linked Lists.(Redis列表是通过链接列表实现的。) This means that even if you have millions of elements inside a list, the operation of adding a new element in the head or in the tail of the list is performed in constant time. The speed of adding a new element with the LPUSH command to the head of a list with ten elements is the same as adding an element to the head of list with 10 million elements.
...
Every time a user posts a new photo, we add its ID into a list with LPUSH.(每次用户发布新照片时，我们都会使用LPUSH将其ID添加到列表中。)
When users visit the home page, we use LRANGE 0 9 in order to get the latest 10 posted items.(当用户访问主页时，我们使用LRANGE 0 9来获取最新发布的10个项目。)
#### Capped lists
...
Redis allows us to use lists as a capped collection, only remembering the latest N items and discarding all the oldest items using the LTRIM command.(Redis允许我们使用列表作为上限集合，仅使用LTRIM命令记住最新的N个项目并丢弃所有最旧的项目。)

The LTRIM command is similar to LRANGE, but instead of displaying the specified range of elements it sets this range as the new list value. All the elements outside the given range are removed.(TRIM命令类似于LRANGE，但是不显示指定的元素范围，而是将范围设置为新列表值。给定范围之外的所有元素都将被删除。)
...
Lists have a special feature that make them suitable to implement queues, and in general as a building block for inter process communication systems: blocking operations.列表具有一项特殊功能，使其适合于实现队列，并且通常用作进程间通信系统的构建块：阻塞操作。
...
However it is possible that sometimes the list is empty and there is nothing to process, so RPOP just returns NULL. In this case a consumer is forced to wait some time and retry again with RPOP. This is called polling（轮询）, and is not a good idea in this context because it has several drawbacks:

1. Forces Redis and clients to process（处理） useless commands (all the requests when the list is empty will get no actual work done, they'll just return NULL).
2. Adds a delay to the processing of items, since after a worker receives a NULL, it waits some time. To make the delay smaller, we could wait less between calls to RPOP, with the effect of amplifying（放大） problem number 1, i.e. more useless calls to Redis.
...
（如果列表为空，它们将能够阻止：仅当将新元素添加到列表中或用户指定的超时时间到时，它们才会返回到调用方到达。）
...
1. When we add an element to an aggregate data type, if the target key does not exist, an empty aggregate data type is created before adding the element.(当我们将元素添加到聚合数据类型时，如果目标键不存在，则在添加元素之前会创建一个空的聚合数据类型。)
2. When we remove elements from an aggregate data type, if the value remains empty, the key is automatically destroyed. The Stream data type is the only exception to this rule.（当我们从聚合数据类型中删除元素时，如果该值保持为空，则键将自动销毁。Stream 数据类型是此规则的唯一例外。）
...
```
HASH 类型的相关命令
hset hget hmset hmget hgetall hincrby 
```
...
4. 2
```
Note: in the example we assume you have another data structure, for example a Redis hash, which maps tag IDs to tag name
...
```
SET 类型的相关命令
sadd srem smembers sismember scard(返回集合中元素的个数)
srandmember key-name [count] 从集合中返回一个或者多个元素
spop 随机的移除集合中的一个元素

组合和处理多个集合的命令
sdiff key-name1 [key-name] 返回只存在第一个集合的元素
sdiffstore destination key-name1 [key-name]  将只存在第一个集合的元素存款到destination
sinter 交集
sinterstore
sunion 并级
sunionstore
...
Implementation note: Sorted sets are implemented via a dual-ported data structure containing both a skip list and a hash table(排序集通过包含跳过列表和哈希表的双端口数据结构实现), so every time we add an element Redis performs an O(log(N)) operation. That's good, but when we ask for sorted elements Redis does not have to do any work at all, it's already all sorted:
```
> zrange hackers 0 -1
1) "Alan Turing"
2) "Hedy Lamarr"
...
3) "Linus Torvalds"
```
Ranges can be inclusive or exclusive (depending on the first character), also string infinite and minus infinite are specified respectively with the + and - strings. See the documentation for more information.(范围可以是包含（inclusive）或排除（exclusive）（取决于第一个字符），字符串无限和负无限分别用+和-字符串指定。有关更多信息，请参见文档。)
...
zrank 
zscore 
zrange key-name start stop 返回集合中排名介于start和stop之间的成员
zrangebyscore key-name min max 返回集合中分数介于min和max之间的成员 
zrevrange 
zrevrangebyscore
...
``` 
#### Bitmaps
Bitmaps are not an actual data type, but a set of bit-oriented operations defined on the String type.(bitmap不是实际的数据类型，而是在String类型上定义的一组面向位的操作) Since strings are binary safe blobs and their maximum length is 512 MB, they are suitable to set up to 232 different bits.
...
One of the biggest advantages of bitmaps is that they often provide extreme space(极限空间) savings when storing information. For example in a system where different users are represented by incremental user IDs, it is possible to remember a single bit information (for example, knowing whether a user wants to receive a newsletter) of 4 billion of users using just 512 MB of memory.(仅使用512 MB内存就可以记住40亿用户的一点信息（例如，知道用户是否要接收新闻通讯）。)
...
1. BITOP performs bit-wise operations between different strings. The provided operations are AND, OR, XOR and NOT.(BITOP在不同的字符串之间执行按位运算。提供的运算为AND，OR，XOR和NOT。)
2. BITCOUNT performs population counting, reporting the number of bits set to 1.(BITCOUNT执行填充计数，报告设置为1的位数。)
3. BITPOS finds the first bit having the specified value of 0 or 1.(BITPOS查找指定值为0或1的第一位。)
...
- Real time analytics of all kinds.(各种实时分析。 )
- Storing space efficient but high performance boolean information associated with object IDs.(存储与对象ID相关的空间高效但高性能的布尔信息。)
